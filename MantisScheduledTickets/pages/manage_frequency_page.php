 ...
 */
    ...
    print_manage_menu();
    print_scheduled_tickets_menu( MST_MANAGE_FREQUENCY_PAGE );
    ...
    if( is_array( $t_frequencies ) ) {
        foreach( $t_frequencies as $t_frequency ) {
            ...
            if( 0 == $t_frequency['enabled'] ) {
                ...
                if( array_key_exists( $t_frequency['id'], $t_crontab_entries ) ) {
        ...
        ?>
            <tr <?php echo helper_alternate_class( $i ) ?>>
                <td>
                    <a href="<?php echo $t_frequency_edit_page . $t_frequency['id']; ?>">
                        <?php echo $t_strike_start; echo $t_frequency['name']; echo $t_strike_end; ?>
                    </a>
                </td>
                <td><?php echo frequency_helper_day_of_week_column( $t_frequency['day_of_week'] ); ?></td>
                <td><?php echo frequency_helper_month_column( $t_frequency['month'] ); ?></td>
                <td><?php echo frequency_helper_day_of_month_column( $t_frequency['day_of_month'] ); ?></td>
                <td><?php echo frequency_helper_hour_column( $t_frequency['hour'] ); ?></td>
                <td><?php echo frequency_helper_minute_column( $t_frequency['minute'] ); ?></td>
                <td class="<?php echo $t_crontab_class; ?>"><?php echo $t_crontab_status; ?></td>
                <td class="<?php echo $t_template_count_class; ?>"><?php echo $t_frequency['template_count']; ?></td>
                <td class="<?php echo $t_bug_count_class; ?>"><?php echo $t_frequency['bug_count']; ?></td>
            </tr>
	<?php
        }
	}
